...
export function MatchCard({ match }: { match: MatchListItem }) {
  ...
  const is2026_27 = match.season_id === 32 || (match.match_date && (match.match_date.startsWith('2026') || match.match_date.startsWith('2027')));
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 sm:w-32 shrink-0">
        ...
        {is2026_27 && (
          <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[#3B82F6]/15 text-[#3B82F6] border border-[#3B82F6]/30">
            AI Pred
          </span>
        )}
      </div>
      ...
      <div className="flex-1 flex items-center justify-between sm:justify-center gap-4">
        <div className="flex-1 text-left sm:text-right">
          <span className="text-sm font-bold text-[#F4F5F2] group-hover:text-[#54C878] transition-colors truncate block">
            {match.home_team.name}
          </span>
        </div>

        <div className="flex flex-col items-center gap-0.5 shrink-0 px-2 min-w-[70px]">
          {played ? (
      ...
      <div className="text-right sm:w-36 shrink-0">
        <span className="text-[11px] font-medium text-[#9DA4AA] block">
          {formattedDay}
        </span>
      </div>
    </Link>
