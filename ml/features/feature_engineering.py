...
def compute_features(df: pd.DataFrame, standings: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    for _, match in df.iterrows():
        ...
        row = {
            ...
            "result": match.get("result"),
            "odds_home": match.get("odds_home", 0.0),
            "odds_draw": match.get("odds_draw", 0.0),
            "odds_away": match.get("odds_away", 0.0),
            "market_prob_home": match.get("market_prob_home", 0.0),
            "market_prob_draw": match.get("market_prob_draw", 0.0),
            "market_prob_away": match.get("market_prob_away", 0.0),
            # Home features
